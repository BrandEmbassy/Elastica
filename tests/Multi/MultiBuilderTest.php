...
class MultiBuilderTest extends BaseTest
{
    ...
    public function testBuildEmptyMultiResultSet(): void
    {
        ...
        $result = $this->multiBuilder->buildMultiResultSet($response, $searches, ApiVersion::API_VERSION_7);
    ...
    public function testBuildMultiResultSet(): void
    {
        ...
        $resultSet1 = new ResultSet(new Response([]), $s1->getQuery(), [], ApiVersion::API_VERSION_7);
        $resultSet2 = new ResultSet(new Response([]), $s2->getQuery(), [], ApiVersion::API_VERSION_7);

        $this->builder->expects($this->exactly(2))
            ->method('buildResultSet')
            ->withConsecutive(
                [$this->isInstanceOf(Response::class), $s1->getQuery(), ApiVersion::API_VERSION_7],
                [$this->isInstanceOf(Response::class), $s2->getQuery(), ApiVersion::API_VERSION_7]
            )
