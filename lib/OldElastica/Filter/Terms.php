...
class Terms extends AbstractFilter
{
     ...
     *
     * @param string                       $key     terms key
     * @param string|\OldElastica\Type        $type    document type from which to fetch the terms values
     ...
     * @param string|array|\OldElastica\Index $options An array of options or the index from which to fetch the terms values. Defaults to the current index.
     *
     * @return $this
     */
    public function setLookup($key, $type, $id, $path, $options = array())
    {
        ...
        if (!is_null($index)) {
            if ($index instanceof \OldElastica\Index) {
                $index = $index->getName();
            }
     ...
     *
     * @see \OldElastica\Filter\AbstractFilter::toArray()
     *
