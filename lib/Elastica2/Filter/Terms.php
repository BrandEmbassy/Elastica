...
class Terms extends AbstractFilter
{
     ...
     *
     * @param string                       $key     terms key
     * @param string|\Elastica2\Type        $type    document type from which to fetch the terms values
     ...
     * @param string|array|\Elastica2\Index $options An array of options or the index from which to fetch the terms values. Defaults to the current index.
     *
     * @return $this
     */
    ...
    {
        $this->_key = $key;
        if ($type instanceof \Elastica2\Type) {
            $type = $type->getName();
        }
        $this->_terms = array(
            'type' => $type,
        ...
        if (!is_null($index)) {
            if ($index instanceof \Elastica2\Index) {
                $index = $index->getName();
            }
     ...
     *
     * @see \Elastica2\Filter\AbstractFilter::toArray()
     *
